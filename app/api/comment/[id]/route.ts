...
export async function DELETE(request: NextRequest, { params }: Props) {
    try {
        ...
        const deletedComment = await Comment.findOneAndDelete({ _id: commentId, userId: session.user.id });
        return NextResponse.json({ message: 'Comment deleted successfully', data: deletedComment }, { status: 200 });
    } catch (error) {
        console.error('Error fetching comments:', error);
        return NextResponse.json(
            { error: 'Failed to fetch comments' },
